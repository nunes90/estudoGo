// ex4.12 - Controlling internal slice behavior
package main
...
func noLink() (int, int) {
	...
	s1 = append(s1, 6) // s1 is now [1, 2, 3, 4, 5, 6]

	s1[3] = 99 // s1[3] is now 99
	return s1[3], s2[3]
}
...
func copyNoLink() (int, int, int) {
	s1 := []int{1, 2, 3, 4, 5}
	s2 := make([]int, len(s1))
	copied := copy(s2, s1) // copy s1 to s2, return number of elements copied
	s1[3] = 99
